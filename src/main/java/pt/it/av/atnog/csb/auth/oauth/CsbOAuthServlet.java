 ...
 */
public class CsbOAuthServlet extends HttpServlet {
	private static final long serialVersionUID = 3083924242786185155L;

	private final static Logger logger = LoggerFactory.getLogger(CsbOAuthServlet.class);
     ...
     * Relative path for the token authorization confirmation URL 
     */
    final static String TOKEN_AUTHORIZATION_CONFIRM_URL = "/authorize/confirm";
    ...
    throws ServletException {
		...
		provider = CsbOAuthUtils.getOAuthProvider(context);
		validator = CsbOAuthUtils.getValidator(context, provider);
	...
	// cgoncalves:
	private void requestEndUserConfirmation(HttpServletRequest req, HttpServletResponse resp,
			org.jboss.resteasy.auth.oauth.OAuthConsumer consumer, OAuthRequestToken requestToken, String format) throws IOException, OAuthException {
	    ...
	    if ("xml".equals(format))
	    {
	    	...
	    	if (provider.hasUserGivenConsumerAccess(consumer, req.getUserPrincipal().getName())) {
	    		serveTokenAuthorizationConfirmation(req, resp);
			} else {
				...
				try {
					...
					// req.setAttribute("oauth_consumer_scopes",
					// requestToken.getScopes());
					// req.setAttribute("oauth_consumer_permissions",
	...
	private void serveTokenAuthorizationConfirmation(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		logger.trace("Consumer registration");
        
        try{
            String[] values = req.getParameterValues(OAuth.OAUTH_TOKEN);
            if (values == null || values.length != 1) {
            	logger.error("OAUTH_TOKEN is null or empty!");
                resp.setStatus(HttpURLConnection.HTTP_BAD_REQUEST);
                return;
            }
            String requestTokenKey = values[0];
...
//            if (values == null || values.length != 1) {
//            	logger.error("xoauth_end_user_decision is null or empty!");
//                resp.setStatus(HttpURLConnection.HTTP_BAD_REQUEST);
//                return;
//            }
//            
//            boolean authorized = "yes".equals(values[0]) || "true".equals(values[0]);
    		...
    		logger.trace("User {} is giving consumer with key {} access to his data", req.getUserPrincipal().getName(), consumer.getKey());
    		
            if (authorized) // FIXME
            {
                String callback = requestToken.getCallback();
                String verifier = provider.authoriseRequestToken(consumer.getKey(), requestToken.getToken(), req.getUserPrincipal().getName());
